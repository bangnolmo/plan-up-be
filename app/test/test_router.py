...
def setup_module():
    """공통 데이터 준비"""
    client.post("/timeTable", json={"name": "test", "year": 2021, "semester": 1, "owner": "test@test.com"})
...
def test_put_time_table_lectures(headers):
    response = client.put("/timeTable/lectures", json={"table_idx": 1, "class_idx": 1, "sub_num": 1}, headers=headers)
    assert response.status_code == 200
...
def test_delete_time_table_lectures(headers):
    response = client.delete("/timeTable/lectures", params={"table_idx": 1, "class_idx": 1}, headers=headers)
    assert response.status_code == 200
